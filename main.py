...
def quiz_interface(topic, questions):
    ...
    if idx >= len(st.session_state.shuffled_questions):
        ...
        st.info(
            f"✅ Correct Answers: {st.session_state.correct_count} out of {len(st.session_state.shuffled_questions)}"
        )
    ...
    st.session_state.selected_option = st.radio(
        "Choose an answer", q["options"], index=None, key=f"radio_{idx}"
    )
...
def main():
    ...
    menu = [
        "Take Quiz",
        "Track Progress",
        "\U0001f4c5 Smart Study Scheduler",
        "🤖 Ask DSA Doubt",
        "\U0001f512 Logout",
    ]
